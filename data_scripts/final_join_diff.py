...
def main():
    ...
    curr_match_data.run(source_dir = '../tennis_MatchChartingProject/', out_filename = './in_game_data/curr_match_data_diff_model.csv')
    print('\nDone.')
    ...
    curr_match_data = pd.read_csv('./in_game_data/curr_match_data_diff_model.csv', delimiter=",",quoting=3, error_bad_lines=False, encoding = "ISO-8859-1")

    joined_data = pd.merge(hist_data, curr_match_data, how="inner",on=['match_id','player'])
    joined_data.to_csv('./final_joined_data_diff.csv', sep=",",quoting=3, encoding = "ISO-8859-1")

    print("Done. Output file: ./final_joined_data_diff.csv")
